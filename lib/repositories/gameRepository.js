...
const GameRepository = {
  ...
  async get (id) {
    ...
    SELECT g.id, g.width, g.height, g.numMines, g.startTime, g.status, g.mines, g.opens,
      IF(COUNT(f.x) = 0, JSON_ARRAY(), JSON_ARRAYAGG(JSON_OBJECT('x', f.x, 'y', f.y))) as flags
    FROM games g LEFT JOIN flags f ON f.gameId = g.id
    WHERE g.id = ?`, [id])
    console.log(sql)

    // eslint-disable-next-line no-unused-vars
    const [records, fields] = await connection.query(sql)
