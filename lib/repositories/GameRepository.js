...
class GameRepository extends BaseRepository {
  ...
  async startGame (game) {
    await this.query('UPDATE `games` SET status = ?, startTime = ? WHERE id = ?',
      [game.status.toRecord(), new Date(game.stopWatch.startTime), game.id])
  }
  ...
  async endGame (game) {
    await this.query('UPDATE `games` SET status = ?, endTime = ? WHERE id = ?',
      [game.status.toRecord(), new Date(game.stopWatch.endTime), game.id])
  }
  ...
  restore (record) {
    ...
    instance.stopWatch.startTime = new Date(record.startTime).getTime()
    instance.stopWatch.endTime = new Date(record.endTime).getTime()
