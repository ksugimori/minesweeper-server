...
class GameRepository extends BaseRepository {
  ...
  async get (id) {
    ...
    SELECT id, width, height, numMines, startTime, status, 
      (SELECT IF(COUNT(x) = 0, JSON_ARRAY(), JSON_ARRAYAGG(JSON_OBJECT('x', x, 'y', y))) FROM mines WHERE gameId = ? ) as mines,
      (SELECT IF(COUNT(x) = 0, JSON_ARRAY(), JSON_ARRAYAGG(JSON_OBJECT('x', x, 'y', y))) FROM open_cells WHERE gameId = ? ) as opens,
      (SELECT IF(COUNT(x) = 0, JSON_ARRAY(), JSON_ARRAYAGG(JSON_OBJECT('x', x, 'y', y))) FROM flags WHERE gameId = ? ) as flags
    FROM games 
