...
const MineRepository = require('../repositories/MineRepository')
...
const OpenCellController = {
  ...
  async create (req, res, next) {
    dbUtils.doTransaction(async connection => {
      ...
      const mineRepository = MineRepository.from(connection)
      ...
      // 初回クリックなら地雷を登録
      if (beforeStatus === Status.INIT) {
        const mines = game.field.points(cell => cell.isMine)
        await mineRepository.createAll(game.id, mines)
      }
